...
add_shortcode('kast_complete_registration', function () {
    ...
    $subscriptions = function_exists('pms_get_member_subscriptions')
        ? pms_get_member_subscriptions($user_id)
        : [];
    ...
    if (!empty($subscriptions)) {
        foreach ($subscriptions as $sub) {
            if (!empty($sub->subscription_plan_id)) {
                ...
                // AGENCY PLANS
                if (in_array($sub->subscription_plan_id, [3852, 3851])) {
                    $has_agency = true;
                }

                // ARTIST PLANS
                if (in_array($sub->subscription_plan_id, [3850, 3849])) {
                    $has_artist = true;
                }
            }
        }
