...
add_shortcode('kast_find_auditions', function() {
    ...
    <style>
        ...
        .kast-search-bar { background: #666; border: 1px solid #333; padding: 15px; border-radius: 8px; display: flex; gap: 10px; margin-bottom: 30px; flex-wrap: wrap; }
        .kast-input, .kast-select { background: #000; border: 1px solid #444; color: #666; padding: 10px; flex: 1; min-width: 150px; border-radius: 4px; }
        ...
        .kc-label { color: #888; font-size: 13px; }
            ...
            <?php if ($query->have_posts()) : ?>
                ...
                <div class="kast-card">
                    ...
                    <div class="kc-data-list">
                        ...
                        <div class="kc-row">
                            <span class="kc-label">Shooting Location</span>
...
// 1. बैकएंड AJAX फंक्शन - लॉग-इन यूजर्स के लिए
add_action('wp_ajax_kast_apply_audition', 'kast_handle_apply_ajax');

function kast_handle_apply_ajax() {
    // सिक्योरिटी चेक (लॉग-इन है या नहीं)
    if (!is_user_logged_in()) {
        wp_send_json_error('Please login to apply.');
    }
    ...
    if (!$post_id) {
        wp_send_json_error('Invalid Audition ID.');
    }

    // पुराने एप्लिकेंट्स की लिस्ट निकालें
    $applicants = get_post_meta($post_id, '_kast_applicants', true) ?: [];

    // चेक करें कि क्या पहले ही अप्लाई कर चुका है
    if (in_array($user_id, $applicants)) {
        wp_send_json_error('You have already applied.');
    }

    // नया यूजर ID लिस्ट में जोड़ें
    $applicants[] = $user_id;
    update_post_meta($post_id, '_kast_applicants', $applicants);

    // सफलता का मैसेज भेजें
    wp_send_json_success('Successfully applied!');
}
